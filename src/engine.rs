...
use crate::gear::{
    ...
    chord::{並擊機關, 並擊機關輸出信號},
    input::{檔位, 焦點事件處理機關, 觸鍵消息, 輸入事件處理機關},
...
pub fn 微觀引擎() -> 微觀引擎輸出信號 {
    ...
    let 視圖落鍵動作 = Callback::new(move |鍵碼: KeyCode| {
        // 觸屏點擊時, 默認未按下修飾鍵
        let 虛擬檔位 = 檔位::default();
        // 攔截功能鍵
        if 處理功能鍵(觸鍵消息 {
    ...
    let 視圖抬鍵動作 = Callback::new(move |鍵碼: KeyCode| {
        既然抬鍵(鍵碼);
    });
...
#[derive(Clone)]
pub struct 視圖輸入信號 {
    pub 視圖落鍵動作: Callback<KeyCode>,
    pub 視圖抬鍵動作: Callback<KeyCode>,
}
