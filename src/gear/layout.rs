...
impl 配列 {
    ...
    pub fn 縱向交錯(&self) -> bool {
        matches!(self, 配列::縱向錯列分體)
    }
    pub fn 直列(&self) -> bool {
        matches!(
            self,
...
pub fn 佈局機關(
    方案: &輸入方案機關輸出信號,
    已選配列: Signal<Option<配列>>,
    選用配列: 選用配列動作,
) -> 佈局機關輸出信號 {
    let 方案定義 = 方案.方案定義;
    let 實際配列 =
        Signal::derive(move || 已選配列.read().unwrap_or_else(|| 方案定義().佈局.首選配列));

    let (當選盤面, 選擇盤面) = signal(方案定義.get_untracked().佈局.默認盤面);
