...
#[define_opaque(清空動作, 回退動作, 擊鍵動作, 切盤動作)]
pub fn 連擊機關(
    方案: &輸入方案機關輸出信號,
    作業: &作業機關輸出信號,
    佈局: &佈局機關輸出信號,
    ...
    let 碼段起始鍵 =
        move |鍵面: &選擇鍵面| 方案.read().查起始鍵(|鍵位| 鍵面.包含鍵位(鍵位)).is_some();
